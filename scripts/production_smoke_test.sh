...
# Production Smoke Test - Tests production mode (not PERF_TEST=1)
# This script validates that production mode meets the requirements:
# - P95 latency ≤ 50ms
# - Throughput ≥ 1000 rps
...
echo "Testing production mode (PERF_TEST=0) to ensure requirements are met"
...
# Test configurations for production mode
TEST_CONFIGS=(
    "10:1000"   # 10 concurrent, 1000 requests
    "20:2000"   # 20 concurrent, 2000 requests  
    "30:3000"   # 30 concurrent, 3000 requests
    "40:4000"   # 40 concurrent, 4000 requests
    "50:5000"   # 50 concurrent, 5000 requests
    "60:6000"   # 60 concurrent, 6000 requests
)

# Create results directory
mkdir -p benchmark_reports
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
REPORT_FILE="benchmark_reports/production_smoke_report_${TIMESTAMP}.md"
...
USER_EMAIL="production-test-$(date +%s)@example.com"
USER_PASSWORD="ProductionTest123!"

# Register user
REGISTER_RESPONSE=$(curl -s -X POST "$SERVICE_URL/internal/auth/register" \
    -H "Content-Type: application/json" \
    -d "{\"email\":\"$USER_EMAIL\",\"password\":\"$USER_PASSWORD\",\"full_name\":\"Production Test\",\"role\":\"user\"}")
...
UPGRADE_RESPONSE=$(curl -s -X PUT "$SERVICE_URL/internal/users/$USER_EMAIL/subscription" \
    -H "Content-Type: application/json" \
    -d "{\"plan_type\":89,\"payment_reference\":\"PROD-TEST-$(date +%s)\"}")
