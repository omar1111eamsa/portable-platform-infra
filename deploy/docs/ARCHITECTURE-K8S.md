...
├── base/                  # Namespace
├── infra/                 # PostgreSQL, Redis, Consul, RabbitMQ, MinIO
...
| Fichier | Rôle |
|---------|------|
| **minio/pvc.yaml** | PVC persistant pour l’object storage des logs Airflow. |
| **minio/deployment.yaml** | 1 replica, nodeSelector backend-vm, endpoint S3-compatible interne pour les logs distants Airflow. |
| **minio/service.yaml** | ClusterIP 9000. |
| **minio/init-bucket-job.yaml** | Job qui crée le bucket `airflow-logs` au bootstrap. |
...
| Fichier | Rôle |
|---------|------|
| **metamodel-orchestration/deployment.yaml** | API server Airflow (1 replica), nodeSelector backend2, tolère DiskPressure. Airflow configuré en **CeleryExecutor** (Redis broker). Métadonnées Airflow : PostgreSQL via secret `metamodel-db-credentials`. Logs de tâches en remote logging vers MinIO via `aws_default`. |
| **metamodel-orchestration/scheduler-deployment.yaml** | Scheduler Airflow (1 replica) sur backend2 (CeleryExecutor + Redis broker + remote logging MinIO). |
| **metamodel-orchestration/dag-processor-deployment.yaml** | DAG processor Airflow 3 (1 replica) sur backend2 (CeleryExecutor + Redis broker + remote logging MinIO). |
| **metamodel-orchestration/worker-deployment.yaml** | Celery worker Airflow (1 replica) sur backend2. Exécute les tâches asynchrones du DAG et écrit ses logs dans MinIO au lieu de dépendre du log server worker `8793`. |
...
| **PersistentVolumeClaim** | postgres (5Gi), chatbot (1Gi), minio (logs Airflow). |
...
- **minio-credentials** : accès MinIO (`MINIO_ROOT_USER`, `MINIO_ROOT_PASSWORD`, `AIRFLOW_LOGS_BUCKET`).
- **metamodel-airflow-s3-logging** : connexion Airflow `aws_default` vers l’endpoint S3-compatible MinIO.
